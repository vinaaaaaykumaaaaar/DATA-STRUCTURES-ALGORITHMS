...
uint32_t ARMCore::shift(uint32_t op, int shift_type, uint32_t amount)
{
    ...
    {
    case SHIFT_LSL:
        return lsl(op, amount);
    case SHIFT_LSR:
        return lsr(op, amount);
    case SHIFT_ASR:
        return asr(op, amount);
    case SHIFT_ROR:
        return ror(op, amount);
    }
}
...
void ARMCore::doLoadStoreMultiple()
{
    ...
    for (int rrn = 0; rrn <= 15; rrn++)
    {
        int rn = rrn;
        if (!in->lsm.U)
            rn = 15 - rn; // reverse the order, hacky a little
        if ((rl >> rn) & 1)
        {
            if (in->lsm.P)
                addr += 4 * ((in->lsm.U) ? 1 : -1);
...
{
    if ((opcode & 0x0FF000D0) == 0x01200010)
    { // BX or BLX
        if (opcode & 0x20)
        {
            set32(R(REG_LR), PC - 4);
